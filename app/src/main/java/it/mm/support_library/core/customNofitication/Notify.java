package it.mm.support_library.core.customNofitication;
...
import it.mm.support_library.core.customNofitication.helpers.BitmapHelper;
import it.mm.support_library.core.customNofitication.helpers.ResourcesHelper;
...
public class Notify {
    ...
    private Notify(Context _context) {
        ...
        try {
            this.channelId = ResourcesHelper.getStringResourceByKey(context, ChannelData.ID);
        } catch (Resources.NotFoundException e) {
            this.channelId = "NotifyAndroid";
        }

        try {
            this.channelName = ResourcesHelper.getStringResourceByKey(context, ChannelData.NAME);
        } catch (Resources.NotFoundException e) {
            this.channelName = "NotifyAndroidChannel";
        }

        try {
            this.channelDescription = ResourcesHelper.getStringResourceByKey(context, ChannelData.DESCRIPTION);
        } catch (Resources.NotFoundException e) {
    ...
    public static Notify build(@NonNull Context context) {
    ...
    public void show() {
        ...
        if (picture != null) {
            ...
            if (picture instanceof String)
                pictureBitmap = BitmapHelper.getBitmapFromUrl(String.valueOf(picture));
        ...
        if (this.action != null) {
            PendingIntent pi;
            if (SDK_INT >= M)
                pi = PendingIntent.getActivity(context, id, this.action, FLAG_CANCEL_CURRENT | FLAG_IMMUTABLE);
            else pi = PendingIntent.getActivity(context, id, this.action, FLAG_CANCEL_CURRENT);
